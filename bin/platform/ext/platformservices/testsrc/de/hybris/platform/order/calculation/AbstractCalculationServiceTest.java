...
public abstract class AbstractCalculationServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testApplyTaxes() throws CalculationException
	{
		...
		final TaxValue tax_25Percent_applied = tax_25Percent.apply(oe1.getQuantity().doubleValue(),
				oe1.getTotalPrice().doubleValue(), orderModel.getCurrency().getDigits().intValue(),
				orderModel.getNet().booleanValue(), orderModel.getCurrency().getIsocode());
	...
	public void testConvertPriceNetGross()
	{
		...
		final PriceValue resultGross = ((DefaultCalculationService) calculationService).convertPriceIfNecessary(basePriceNet, false,
				curr, taxValues);
	...
	public void shouldRecalculateOrderWithGivenDateWhenQuantityChanged() throws CalculationException
	{
		...
		//when
		calculationService.recalculate(orderModel); //try recalculate without date parameter, should be set to calculated afterwards
